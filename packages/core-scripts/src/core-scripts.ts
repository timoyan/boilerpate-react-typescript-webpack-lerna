...
let analyzer = argv.analyzer || false;
...
function executeCompile(port, host) {
  const compiler = Webpack(config(env, isBuild, host, port, analyzer));
...
function executeDevServer(port, host) {
  const compiler: any = Webpack(config(env, isBuild, host, port, analyzer));
